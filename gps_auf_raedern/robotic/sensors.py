"""
This module shall start a background instance of advanced sensor-control
...
class GPSCarSensors(threading.Thread):
	...
	time.sleep(0.00001)
        ...
        try:  # added because of error 'referrenced before assignement'
            ...
            self.is_at_measure = False
            lg.prt("exception from sensor " + str(sensor_number + 1) + " of 3", inst=__name__,
                   lv=10000)  # + "): unknown error from sensor (probably \'referenced before assignment\')"
            GPIO.output(self.TRIG[sensor_number], False)
            time.sleep(0.2)
            self.lock.release()
    ...
    def move_servo(self, value, percentage=-1):
        ...
        if percentage == -1:
            ...
            self.servo.set_servo(self.SERVO, value)
        ...
        # ether by percentage (value between 0 and 1), if given (0 = MIN, 1 = MAX)
            ...
            self.servo.set_servo(self.SERVO, self.servo_position)
    ...
    def run(self):
        ...
        while self.running is True:
            ...
            #          servo scans: from left to right to left to right... starting at center
                ...
                for i in range(4):
                    ...
                    if cycle[i] == 1:  # move servo to next position
                        ...
                        elif ((j == self.servo_segments) and (
                                    servo_direction == +1)):  # servo is at right end and wants to move further right
            ...
            #          like mode 2, but averaging over 2 consecutive measurements
                ...
                lg.prt("Unknown mode set to sensor-scanning-thread! Waiting for corrent mode...", lv=100000,
                       inst=__name__)
