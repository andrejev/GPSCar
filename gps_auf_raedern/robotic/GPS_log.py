...
lg = debug_print(debug_level=150, save_level=0, save_debug=True,
                 filename="debug_print/debugprint" + str(time.time()) + ".txt", time_stamp=False)
...
class gpslog:
    ...
    def add_log(self, current_status, GPS_data):
        if (self.opened == False):
            lg.prt("Warning: No file opened to write to! Probably it was closed before. Openening file again.",
                   lv=10000, inst=__name__)
            self.log_file = open(log_file_name, 'a')
        gps_log = ''
        for entry in GPS_data:
            gps_log = gps_log + str(
                entry) + self.data_separator  # concentrate GPS-data to string with spaces as separation
        ...
        current_time = time.strftime(
            "%Y" + self.data_separator + "%m" + self.data_separator + "%d" + self.data_separator + "%H" + self.data_separator + "%M" + self.data_separator + "%S" + self.data_separator,
            lt)  # concentrate local time to string

        log_entry = str(
            time.time()) + self.data_separator + current_time + gps_log + status_log  # generate output string with all data (spaces for seperation)
