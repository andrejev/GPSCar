...
class GpsPoller(threading.Thread):
    ...
    def __init__(self, start=True):
        ...
        self.degree_offset = -70  # offset to north in degree
    ...
    def gps_routine(self):
        ...
        bearing %= 360  # normalize to 0..360
        ...
        if [gpsd.fix.latitude, gpsd.fix.longitude] != [self.lat_list[(self.i - 1) % self.average_number],
                                                       self.long_list[(
                                                                   self.i - 1) % self.average_number]]:  # check whether new data is available
            ...
            self.data = [sum(self.lat_list) / float(len(self.lat_list)),
                         sum(self.long_list) / float(len(self.long_list)),
                         sum(self.alt_list) / float(len(self.alt_list)), bearing, gpsd.satellites, time.time()]
            self.i += 1

            # save raw_data also (not averaged)
            self.raw_data = [gpsd.fix.latitude, gpsd.fix.longitude, gpsd.fix.altitude, bearing, gpsd.satellites,
                             time.time()]

            dist = math.sqrt((self.raw_data[0] - self.last[0]) * (self.raw_data[0] - self.last[0]) + (
                self.raw_data[1] - self.last[1]) * (self.raw_data[1] - self.last[1]))
    ...
    def run(self):
        ...
        while self.running:
            if self.is_used:
                self.gps_routine()
            # it takes about 0.25s for the gps to deliver new data
            time.sleep(0.05)
